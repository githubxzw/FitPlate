  ...
  return (
    <button
      ...
      className={cn(
        ...
        checked
          ? "bg-gradient-to-b from-brand-500 to-brand-600 text-white shadow-brand hover:from-brand-500 hover:to-brand-700"
    ...
    >
      {checked ? (
        <span key="on" aria-hidden className="inline-block animate-pop-in">
          ✓
        </span>
      ) : (
...
export function Stat({ label, value, sub, className }: { label: string; value: React.ReactNode; sub?: string; className?: string }) {
  return (
    <div className={cn("rounded-xl bg-zinc-50 px-3 py-2.5 dark:bg-zinc-800/60", className)}>
      ...
      <div className="tnum mt-0.5 text-lg font-semibold leading-tight">{value}</div>
...
export function EmptyState({
  ...
  return (
    <div className="animate-fade-up flex flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-zinc-300 px-6 py-10 text-center dark:border-zinc-700">
...
export function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-0 sm:items-center sm:p-6" onClick={onClose}>
      <div
        className="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-t-2xl bg-white p-5 shadow-xl dark:bg-zinc-900 sm:rounded-2xl sm:animate-pop-in"
...
export function Toast({ message, tone = "ok" }: { message: string | null; tone?: "ok" | "err" }) {
  ...
  return (
    <div
      className={cn(
        "no-print fixed bottom-24 left-1/2 z-[60] -translate-x-1/2 animate-toast-in rounded-full px-4 py-2 text-sm text-white shadow-lg sm:bottom-6",
